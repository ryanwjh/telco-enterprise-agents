...
### Business Problem
Addressing core telecommunications operational challenges in sim swap fraud prevention api through automated quantitative analytics, predictive AI, and real-time grounding.
...
- **`data_insights`**: Queries internal BigQuery tables (`daas_ssfp_*`) using the BigQuery Conversational Analytics API (`ask_data_insights`, `forecast`, `analyze_contribution`, `detect_anomalies`) and generates visual data charts via `render_chart`.
...
- `daas_ssfp_sim_swap_event_timestamps` — Seeded via `data/sim_swap_event_timestamps.csv`
- `daas_ssfp_banking_api_query_logs` — Seeded via `data/banking_api_query_logs.csv`
- `daas_ssfp_fraud_verification_audit_trail` — Seeded via `data/fraud_verification_audit_trail.csv`

---

## 5. Example Questions

1. "What are our primary operational metrics and performance targets for sim swap fraud prevention api across operating regions in 2026 YTD?"
2. "What are the latest telecom industry standards, GSMA guidelines, and market benchmarks for sim swap fraud prevention api?"
3. "Render a chart comparing monthly performance metrics for sim swap fraud prevention api vs annual targets."
4. "Break down sim swap fraud prevention api volume by operating cluster and customer segment for 2026 YTD."
5. "What are the projected quarterly financial impacts and ROI of optimizing sim swap fraud prevention api?"
...
```bash
# Run unit tests
uv run --frozen pytest domains/daas_camara/agents/sim_swap_fraud_prevention_api/tests/unit -v

# Run interactively with ADK CLI
adk run domains/daas_camara/agents/sim_swap_fraud_prevention_api
```
