...
### Business Problem
Addressing core telecommunications operational challenges in quality on demand qod api through automated quantitative analytics, predictive AI, and real-time grounding.
...
- **`data_insights`**: Queries internal BigQuery tables (`daas_qoda_*`) using the BigQuery Conversational Analytics API (`ask_data_insights`, `forecast`, `analyze_contribution`, `detect_anomalies`) and generates visual data charts via `render_chart`.
...
- `daas_qoda_qod_slice_session_requests` — Seeded via `data/qod_slice_session_requests.csv`
- `daas_qoda_guaranteed_bandwidth_metrics` — Seeded via `data/guaranteed_bandwidth_metrics.csv`
- `daas_qoda_developer_api_billings` — Seeded via `data/developer_api_billings.csv`

---

## 5. Example Questions

1. "What are our primary operational metrics and performance targets for quality on demand qod api across operating regions in 2026 YTD?"
2. "What are the latest telecom industry standards, GSMA guidelines, and market benchmarks for quality on demand qod api?"
3. "Render a chart comparing monthly performance metrics for quality on demand qod api vs annual targets."
4. "Break down quality on demand qod api volume by operating cluster and customer segment for 2026 YTD."
5. "What are the projected quarterly financial impacts and ROI of optimizing quality on demand qod api?"
...
```bash
# Run unit tests
uv run --frozen pytest domains/daas_camara/agents/quality_on_demand_qod_api/tests/unit -v

# Run interactively with ADK CLI
adk run domains/daas_camara/agents/quality_on_demand_qod_api
```
