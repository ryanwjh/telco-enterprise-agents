...
### Business Problem
Addressing core telecommunications operational challenges in number verification api through automated quantitative analytics, predictive AI, and real-time grounding.
...
- **`data_insights`**: Queries internal BigQuery tables (`daas_nvap_*`) using the BigQuery Conversational Analytics API (`ask_data_insights`, `forecast`, `analyze_contribution`, `detect_anomalies`) and generates visual data charts via `render_chart`.
...
- `daas_nvap_camara_auth_token_requests` — Seeded via `data/camara_auth_token_requests.csv`
- `daas_nvap_carrier_billing_id_matches` — Seeded via `data/carrier_billing_id_matches.csv`
- `daas_nvap_api_monetization_ledgers` — Seeded via `data/api_monetization_ledgers.csv`

---

## 5. Example Questions

1. "What are our primary operational metrics and performance targets for number verification api across operating regions in 2026 YTD?"
2. "What are the latest telecom industry standards, GSMA guidelines, and market benchmarks for number verification api?"
3. "Render a chart comparing monthly performance metrics for number verification api vs annual targets."
4. "Break down number verification api volume by operating cluster and customer segment for 2026 YTD."
5. "What are the projected quarterly financial impacts and ROI of optimizing number verification api?"
...
```bash
# Run unit tests
uv run --frozen pytest domains/daas_camara/agents/number_verification_api/tests/unit -v

# Run interactively with ADK CLI
adk run domains/daas_camara/agents/number_verification_api
```
