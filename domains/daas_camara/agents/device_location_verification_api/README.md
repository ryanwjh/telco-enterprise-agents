...
### Business Problem
Addressing core telecommunications operational challenges in device location verification api through automated quantitative analytics, predictive AI, and real-time grounding.
...
- **`data_insights`**: Queries internal BigQuery tables (`daas_dlva_*`) using the BigQuery Conversational Analytics API (`ask_data_insights`, `forecast`, `analyze_contribution`, `detect_anomalies`) and generates visual data charts via `render_chart`.
...
- `daas_dlva_location_verification_queries` — Seeded via `data/location_verification_queries.csv`
- `daas_dlva_geofence_match_results` — Seeded via `data/geofence_match_results.csv`
- `daas_dlva_enterprise_sla_tracking` — Seeded via `data/enterprise_sla_tracking.csv`

---

## 5. Example Questions

1. "What are our primary operational metrics and performance targets for device location verification api across operating regions in 2026 YTD?"
2. "What are the latest telecom industry standards, GSMA guidelines, and market benchmarks for device location verification api?"
3. "Render a chart comparing monthly performance metrics for device location verification api vs annual targets."
4. "Break down device location verification api volume by operating cluster and customer segment for 2026 YTD."
5. "What are the projected quarterly financial impacts and ROI of optimizing device location verification api?"
...
```bash
# Run unit tests
uv run --frozen pytest domains/daas_camara/agents/device_location_verification_api/tests/unit -v

# Run interactively with ADK CLI
adk run domains/daas_camara/agents/device_location_verification_api
```
