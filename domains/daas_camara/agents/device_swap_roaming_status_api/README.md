...
### Business Problem
Addressing core telecommunications operational challenges in device swap roaming status api through automated quantitative analytics, predictive AI, and real-time grounding.

### Target Personas
Head of Telecom Operations, DaaS & CAMARA Director, Principal Network Engineer, CVM Strategy Lead
...
- **`data_insights`**: Queries internal BigQuery tables (`daas_dsrs_*`) using the BigQuery Conversational Analytics API (`ask_data_insights`, `forecast`, `analyze_contribution`, `detect_anomalies`) and generates visual data charts via `render_chart`.
...
- `daas_dsrs_device_imei_change_events` — Seeded via `data/device_imei_change_events.csv`
- `daas_dsrs_international_roaming_registrations` — Seeded via `data/international_roaming_registrations.csv`
- `daas_dsrs_partner_verification_records` — Seeded via `data/partner_verification_records.csv`

---

## 5. Example Questions

1. "What are our primary operational metrics and performance targets for device swap roaming status api across operating regions in 2026 YTD?"
2. "What are the latest telecom industry standards, GSMA guidelines, and market benchmarks for device swap roaming status api?"
3. "Render a chart comparing monthly performance metrics for device swap roaming status api vs annual targets."
4. "Break down device swap roaming status api volume by operating cluster and customer segment for 2026 YTD."
5. "What are the projected quarterly financial impacts and ROI of optimizing device swap roaming status api?"
...
```bash
# Run unit tests
uv run --frozen pytest domains/daas_camara/agents/device_swap_roaming_status_api/tests/unit -v

# Run interactively with ADK CLI
adk run domains/daas_camara/agents/device_swap_roaming_status_api
```
