...
### Business Problem
Addressing core telecommunications operational challenges in stadium arena crowd density through automated quantitative analytics, predictive AI, and real-time grounding.
...
- **`data_insights`**: Queries internal BigQuery tables (`daas_sacd_*`) using the BigQuery Conversational Analytics API (`ask_data_insights`, `forecast`, `analyze_contribution`, `detect_anomalies`) and generates visual data charts via `render_chart`.
...
- `daas_sacd_anonymized_cell_attachments` — Seeded via `data/anonymized_cell_attachments.csv`
- `daas_sacd_venue_geofence_boundaries` — Seeded via `data/venue_geofence_boundaries.csv`
- `daas_sacd_hourly_crowd_density_aggregates` — Seeded via `data/hourly_crowd_density_aggregates.csv`

---

## 5. Example Questions

1. "What are our primary operational metrics and performance targets for stadium arena crowd density across operating regions in 2026 YTD?"
2. "What are the latest telecom industry standards, GSMA guidelines, and market benchmarks for stadium arena crowd density?"
3. "Render a chart comparing monthly performance metrics for stadium arena crowd density vs annual targets."
4. "Break down stadium arena crowd density volume by operating cluster and customer segment for 2026 YTD."
5. "What are the projected quarterly financial impacts and ROI of optimizing stadium arena crowd density?"
...
```bash
# Run unit tests
uv run --frozen pytest domains/daas_camara/agents/stadium_arena_crowd_density/tests/unit -v

# Run interactively with ADK CLI
adk run domains/daas_camara/agents/stadium_arena_crowd_density
```
