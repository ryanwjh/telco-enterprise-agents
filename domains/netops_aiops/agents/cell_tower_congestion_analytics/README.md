...
### Business Problem
Addressing core telecommunications operational challenges in cell tower congestion analytics through automated quantitative analytics, predictive AI, and real-time grounding.
...
- **`data_insights`**: Queries internal BigQuery tables (`ntop_ctca_*`) using the BigQuery Conversational Analytics API (`ask_data_insights`, `forecast`, `analyze_contribution`, `detect_anomalies`) and generates visual data charts via `render_chart`.
...
- `ntop_ctca_ran_sector_prb_utilization` — Seeded via `data/ran_sector_prb_utilization.csv`
- `ntop_ctca_voice_drop_call_rates` — Seeded via `data/voice_drop_call_rates.csv`
- `ntop_ctca_capacity_expansion_priorities` — Seeded via `data/capacity_expansion_priorities.csv`

---

## 5. Example Questions

1. "What are our primary operational metrics and performance targets for cell tower congestion analytics across operating regions in 2026 YTD?"
2. "What are the latest telecom industry standards, GSMA guidelines, and market benchmarks for cell tower congestion analytics?"
3. "Render a chart comparing monthly performance metrics for cell tower congestion analytics vs annual targets."
4. "Break down cell tower congestion analytics volume by operating cluster and customer segment for 2026 YTD."
5. "What are the projected quarterly financial impacts and ROI of optimizing cell tower congestion analytics?"
...
```bash
# Run unit tests
uv run --frozen pytest domains/netops_aiops/agents/cell_tower_congestion_analytics/tests/unit -v

# Run interactively with ADK CLI
adk run domains/netops_aiops/agents/cell_tower_congestion_analytics
```
