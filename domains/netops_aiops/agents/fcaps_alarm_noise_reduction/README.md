...
### Business Problem
Addressing core telecommunications operational challenges in fcaps alarm noise reduction through automated quantitative analytics, predictive AI, and real-time grounding.
...
- **`data_insights`**: Queries internal BigQuery tables (`ntop_fcap_*`) using the BigQuery Conversational Analytics API (`ask_data_insights`, `forecast`, `analyze_contribution`, `detect_anomalies`) and generates visual data charts via `render_chart`.
...
- `ntop_fcap_raw_snmp_alarm_streams` — Seeded via `data/raw_snmp_alarm_streams.csv`
- `ntop_fcap_alarm_correlation_rules` — Seeded via `data/alarm_correlation_rules.csv`
- `ntop_fcap_actionable_incident_clusters` — Seeded via `data/actionable_incident_clusters.csv`

---

## 5. Example Questions

1. "What are our primary operational metrics and performance targets for fcaps alarm noise reduction across operating regions in 2026 YTD?"
2. "What are the latest telecom industry standards, GSMA guidelines, and market benchmarks for fcaps alarm noise reduction?"
3. "Render a chart comparing monthly performance metrics for fcaps alarm noise reduction vs annual targets."
4. "Break down fcaps alarm noise reduction volume by operating cluster and customer segment for 2026 YTD."
5. "What are the projected quarterly financial impacts and ROI of optimizing fcaps alarm noise reduction?"
...
```bash
# Run unit tests
uv run --frozen pytest domains/netops_aiops/agents/fcaps_alarm_noise_reduction/tests/unit -v

# Run interactively with ADK CLI
adk run domains/netops_aiops/agents/fcaps_alarm_noise_reduction
```
