...
### Business Problem
Addressing core telecommunications operational challenges in airport roaming pass through automated quantitative analytics, predictive AI, and real-time grounding.
...
- **`data_insights`**: Queries internal BigQuery tables (`cmkt_arps_*`) using the BigQuery Conversational Analytics API (`ask_data_insights`, `forecast`, `analyze_contribution`, `detect_anomalies`) and generates visual data charts via `render_chart`.
...
- `cmkt_arps_airport_geofence_triggers` — Seeded via `data/airport_geofence_triggers.csv`
- `cmkt_arps_outbound_roaming_passes` — Seeded via `data/outbound_roaming_passes.csv`
- `cmkt_arps_traveler_pack_activations` — Seeded via `data/traveler_pack_activations.csv`

---

## 5. Example Questions

1. "What are our primary operational metrics and performance targets for airport roaming pass across operating regions in 2026 YTD?"
2. "What are the latest telecom industry standards, GSMA guidelines, and market benchmarks for airport roaming pass?"
3. "Render a chart comparing monthly performance metrics for airport roaming pass vs annual targets."
4. "Break down airport roaming pass volume by operating cluster and customer segment for 2026 YTD."
5. "What are the projected quarterly financial impacts and ROI of optimizing airport roaming pass?"
...
```bash
# Run unit tests
uv run --frozen pytest domains/consumer_marketing/agents/airport_roaming_pass/tests/unit -v

# Run interactively with ADK CLI
adk run domains/consumer_marketing/agents/airport_roaming_pass
```
