...
### Business Problem
Addressing core telecommunications operational challenges in international call pack through automated quantitative analytics, predictive AI, and real-time grounding.
...
- **`data_insights`**: Queries internal BigQuery tables (`cmkt_icpk_*`) using the BigQuery Conversational Analytics API (`ask_data_insights`, `forecast`, `analyze_contribution`, `detect_anomalies`) and generates visual data charts via `render_chart`.
...
- `cmkt_icpk_international_cdr_usage` — Seeded via `data/international_cdr_usage.csv`
- `cmkt_icpk_roaming_calling_zones` — Seeded via `data/roaming_calling_zones.csv`
- `cmkt_icpk_discount_rate_plans` — Seeded via `data/discount_rate_plans.csv`

---

## 5. Example Questions

1. "What are our primary operational metrics and performance targets for international call pack across operating regions in 2026 YTD?"
2. "What are the latest telecom industry standards, GSMA guidelines, and market benchmarks for international call pack?"
3. "Render a chart comparing monthly performance metrics for international call pack vs annual targets."
4. "Break down international call pack volume by operating cluster and customer segment for 2026 YTD."
5. "What are the projected quarterly financial impacts and ROI of optimizing international call pack?"
...
```bash
# Run unit tests
uv run --frozen pytest domains/consumer_marketing/agents/international_call_pack/tests/unit -v

# Run interactively with ADK CLI
adk run domains/consumer_marketing/agents/international_call_pack
```
