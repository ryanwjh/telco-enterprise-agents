...
### Business Problem
Addressing core telecommunications operational challenges in work from home broadband through automated quantitative analytics, predictive AI, and real-time grounding.
...
- **`data_insights`**: Queries internal BigQuery tables (`cmkt_wfmb_*`) using the BigQuery Conversational Analytics API (`ask_data_insights`, `forecast`, `analyze_contribution`, `detect_anomalies`) and generates visual data charts via `render_chart`.
...
- `cmkt_wfmb_enterprise_vpn_telemetry` — Seeded via `data/enterprise_vpn_telemetry.csv`
- `cmkt_wfmb_dedicated_wfh_broadband_plans` — Seeded via `data/dedicated_wfh_broadband_plans.csv`
- `cmkt_wfmb_dual_sim_hybrid_orders` — Seeded via `data/dual_sim_hybrid_orders.csv`

---

## 5. Example Questions

1. "What are our primary operational metrics and performance targets for work from home broadband across operating regions in 2026 YTD?"
2. "What are the latest telecom industry standards, GSMA guidelines, and market benchmarks for work from home broadband?"
3. "Render a chart comparing monthly performance metrics for work from home broadband vs annual targets."
4. "Break down work from home broadband volume by operating cluster and customer segment for 2026 YTD."
5. "What are the projected quarterly financial impacts and ROI of optimizing work from home broadband?"
...
```bash
# Run unit tests
uv run --frozen pytest domains/consumer_marketing/agents/work_from_home_broadband/tests/unit -v

# Run interactively with ADK CLI
adk run domains/consumer_marketing/agents/work_from_home_broadband
```
