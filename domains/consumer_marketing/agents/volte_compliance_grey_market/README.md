...
### Business Problem
Addressing core telecommunications operational challenges in volte compliance grey market through automated quantitative analytics, predictive AI, and real-time grounding.
...
- **`data_insights`**: Queries internal BigQuery tables (`cmkt_vcgm_*`) using the BigQuery Conversational Analytics API (`ask_data_insights`, `forecast`, `analyze_contribution`, `detect_anomalies`) and generates visual data charts via `render_chart`.
...
- `cmkt_vcgm_imei_tac_database` — Seeded via `data/imei_tac_database.csv`
- `cmkt_vcgm_network_registration_logs` — Seeded via `data/network_registration_logs.csv`
- `cmkt_vcgm_device_retirement_notices` — Seeded via `data/device_retirement_notices.csv`

---

## 5. Example Questions

1. "What are our primary operational metrics and performance targets for volte compliance grey market across operating regions in 2026 YTD?"
2. "What are the latest telecom industry standards, GSMA guidelines, and market benchmarks for volte compliance grey market?"
3. "Render a chart comparing monthly performance metrics for volte compliance grey market vs annual targets."
4. "Break down volte compliance grey market volume by operating cluster and customer segment for 2026 YTD."
5. "What are the projected quarterly financial impacts and ROI of optimizing volte compliance grey market?"
...
```bash
# Run unit tests
uv run --frozen pytest domains/consumer_marketing/agents/volte_compliance_grey_market/tests/unit -v

# Run interactively with ADK CLI
adk run domains/consumer_marketing/agents/volte_compliance_grey_market
```
