...
### Business Problem
Addressing core telecommunications operational challenges in in the moment data boost through automated quantitative analytics, predictive AI, and real-time grounding.
...
- **`data_insights`**: Queries internal BigQuery tables (`cmkt_imdb_*`) using the BigQuery Conversational Analytics API (`ask_data_insights`, `forecast`, `analyze_contribution`, `detect_anomalies`) and generates visual data charts via `render_chart`.
...
- `cmkt_imdb_subscriber_data_allowances` — Seeded via `data/subscriber_data_allowances.csv`
- `cmkt_imdb_hourly_depletion_velocity` — Seeded via `data/hourly_depletion_velocity.csv`
- `cmkt_imdb_ondemand_topup_campaigns` — Seeded via `data/ondemand_topup_campaigns.csv`

---

## 5. Example Questions

1. "What are our primary operational metrics and performance targets for in the moment data boost across operating regions in 2026 YTD?"
2. "What are the latest telecom industry standards, GSMA guidelines, and market benchmarks for in the moment data boost?"
3. "Render a chart comparing monthly performance metrics for in the moment data boost vs annual targets."
4. "Break down in the moment data boost volume by operating cluster and customer segment for 2026 YTD."
5. "What are the projected quarterly financial impacts and ROI of optimizing in the moment data boost?"
...
```bash
# Run unit tests
uv run --frozen pytest domains/consumer_marketing/agents/in_the_moment_data_boost/tests/unit -v

# Run interactively with ADK CLI
adk run domains/consumer_marketing/agents/in_the_moment_data_boost
```
