...
### Business Problem
Addressing core telecommunications operational challenges in tourist welcome roaming through automated quantitative analytics, predictive AI, and real-time grounding.
...
- **`data_insights`**: Queries internal BigQuery tables (`cmkt_twro_*`) using the BigQuery Conversational Analytics API (`ask_data_insights`, `forecast`, `analyze_contribution`, `detect_anomalies`) and generates visual data charts via `render_chart`.
...
- `cmkt_twro_inbound_roamer_events` — Seeded via `data/inbound_roamer_events.csv`
- `cmkt_twro_tourist_pass_catalog` — Seeded via `data/tourist_pass_catalog.csv`
- `cmkt_twro_multilingual_welcome_conversions` — Seeded via `data/multilingual_welcome_conversions.csv`

---

## 5. Example Questions

1. "What are our primary operational metrics and performance targets for tourist welcome roaming across operating regions in 2026 YTD?"
2. "What are the latest telecom industry standards, GSMA guidelines, and market benchmarks for tourist welcome roaming?"
3. "Render a chart comparing monthly performance metrics for tourist welcome roaming vs annual targets."
4. "Break down tourist welcome roaming volume by operating cluster and customer segment for 2026 YTD."
5. "What are the projected quarterly financial impacts and ROI of optimizing tourist welcome roaming?"
...
```bash
# Run unit tests
uv run --frozen pytest domains/consumer_marketing/agents/tourist_welcome_roaming/tests/unit -v

# Run interactively with ADK CLI
adk run domains/consumer_marketing/agents/tourist_welcome_roaming
```
