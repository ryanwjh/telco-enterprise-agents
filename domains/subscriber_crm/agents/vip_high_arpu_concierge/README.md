...
### Business Problem
Addressing core telecommunications operational challenges in vip high arpu concierge through automated quantitative analytics, predictive AI, and real-time grounding.
...
- **`data_insights`**: Queries internal BigQuery tables (`scrm_vipr_*`) using the BigQuery Conversational Analytics API (`ask_data_insights`, `forecast`, `analyze_contribution`, `detect_anomalies`) and generates visual data charts via `render_chart`.
...
- `scrm_vipr_platinum_subscriber_rosters` — Seeded via `data/platinum_subscriber_rosters.csv`
- `scrm_vipr_dedicated_concierge_tickets` — Seeded via `data/dedicated_concierge_tickets.csv`
- `scrm_vipr_sla_resolution_timelines` — Seeded via `data/sla_resolution_timelines.csv`

---

## 5. Example Questions

1. "What are our primary operational metrics and performance targets for vip high arpu concierge across operating regions in 2026 YTD?"
2. "What are the latest telecom industry standards, GSMA guidelines, and market benchmarks for vip high arpu concierge?"
3. "Render a chart comparing monthly performance metrics for vip high arpu concierge vs annual targets."
4. "Break down vip high arpu concierge volume by operating cluster and customer segment for 2026 YTD."
5. "What are the projected quarterly financial impacts and ROI of optimizing vip high arpu concierge?"
...
```bash
# Run unit tests
uv run --frozen pytest domains/subscriber_crm/agents/vip_high_arpu_concierge/tests/unit -v

# Run interactively with ADK CLI
adk run domains/subscriber_crm/agents/vip_high_arpu_concierge
```
