...
### Business Problem
Addressing core telecommunications operational challenges in payment arrangement promise to pay through automated quantitative analytics, predictive AI, and real-time grounding.
...
- **`data_insights`**: Queries internal BigQuery tables (`scrm_papp_*`) using the BigQuery Conversational Analytics API (`ask_data_insights`, `forecast`, `analyze_contribution`, `detect_anomalies`) and generates visual data charts via `render_chart`.
...
- `scrm_papp_delinquent_account_aging` — Seeded via `data/delinquent_account_aging.csv`
- `scrm_papp_promise_to_pay_commitments` — Seeded via `data/promise_to_pay_commitments.csv`
- `scrm_papp_service_restoration_triggers` — Seeded via `data/service_restoration_triggers.csv`

---

## 5. Example Questions

1. "What are our primary operational metrics and performance targets for payment arrangement promise to pay across operating regions in 2026 YTD?"
2. "What are the latest telecom industry standards, GSMA guidelines, and market benchmarks for payment arrangement promise to pay?"
3. "Render a chart comparing monthly performance metrics for payment arrangement promise to pay vs annual targets."
4. "Break down payment arrangement promise to pay volume by operating cluster and customer segment for 2026 YTD."
5. "What are the projected quarterly financial impacts and ROI of optimizing payment arrangement promise to pay?"
...
```bash
# Run unit tests
uv run --frozen pytest domains/subscriber_crm/agents/payment_arrangement_promise_to_pay/tests/unit -v

# Run interactively with ADK CLI
adk run domains/subscriber_crm/agents/payment_arrangement_promise_to_pay
```
