...
### Business Problem
Addressing core telecommunications operational challenges in bill shock charge breakdown through automated quantitative analytics, predictive AI, and real-time grounding.
...
- **`data_insights`**: Queries internal BigQuery tables (`scrm_bscb_*`) using the BigQuery Conversational Analytics API (`ask_data_insights`, `forecast`, `analyze_contribution`, `detect_anomalies`) and generates visual data charts via `render_chart`.
...
- `scrm_bscb_monthly_billing_cdrs` — Seeded via `data/monthly_billing_cdrs.csv`
- `scrm_bscb_roaming_overage_details` — Seeded via `data/roaming_overage_details.csv`
- `scrm_bscb_goodwill_adjustment_ledgers` — Seeded via `data/goodwill_adjustment_ledgers.csv`

---

## 5. Example Questions

1. "What are our primary operational metrics and performance targets for bill shock charge breakdown across operating regions in 2026 YTD?"
2. "What are the latest telecom industry standards, GSMA guidelines, and market benchmarks for bill shock charge breakdown?"
3. "Render a chart comparing monthly performance metrics for bill shock charge breakdown vs annual targets."
4. "Break down bill shock charge breakdown volume by operating cluster and customer segment for 2026 YTD."
5. "What are the projected quarterly financial impacts and ROI of optimizing bill shock charge breakdown?"
...
```bash
# Run unit tests
uv run --frozen pytest domains/subscriber_crm/agents/bill_shock_charge_breakdown/tests/unit -v

# Run interactively with ADK CLI
adk run domains/subscriber_crm/agents/bill_shock_charge_breakdown
```
