...
### Business Problem
Addressing core telecommunications operational challenges in vas subscription manager through automated quantitative analytics, predictive AI, and real-time grounding.
...
- **`data_insights`**: Queries internal BigQuery tables (`scrm_vass_*`) using the BigQuery Conversational Analytics API (`ask_data_insights`, `forecast`, `analyze_contribution`, `detect_anomalies`) and generates visual data charts via `render_chart`.
...
- `scrm_vass_active_vas_services` — Seeded via `data/active_vas_services.csv`
- `scrm_vass_partner_content_charges` — Seeded via `data/partner_content_charges.csv`
- `scrm_vass_subscription_cancellation_logs` — Seeded via `data/subscription_cancellation_logs.csv`

---

## 5. Example Questions

1. "What are our primary operational metrics and performance targets for vas subscription manager across operating regions in 2026 YTD?"
2. "What are the latest telecom industry standards, GSMA guidelines, and market benchmarks for vas subscription manager?"
3. "Render a chart comparing monthly performance metrics for vas subscription manager vs annual targets."
4. "Break down vas subscription manager volume by operating cluster and customer segment for 2026 YTD."
5. "What are the projected quarterly financial impacts and ROI of optimizing vas subscription manager?"
...
```bash
# Run unit tests
uv run --frozen pytest domains/subscriber_crm/agents/vas_subscription_manager/tests/unit -v

# Run interactively with ADK CLI
adk run domains/subscriber_crm/agents/vas_subscription_manager
```
