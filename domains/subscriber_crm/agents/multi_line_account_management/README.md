...
### Business Problem
Addressing core telecommunications operational challenges in multi line account management through automated quantitative analytics, predictive AI, and real-time grounding.
...
- **`data_insights`**: Queries internal BigQuery tables (`scrm_mlam_*`) using the BigQuery Conversational Analytics API (`ask_data_insights`, `forecast`, `analyze_contribution`, `detect_anomalies`) and generates visual data charts via `render_chart`.
...
- `scrm_mlam_account_line_hierarchies` — Seeded via `data/account_line_hierarchies.csv`
- `scrm_mlam_shared_pool_usages` — Seeded via `data/shared_pool_usages.csv`
- `scrm_mlam_billing_inquiry_summaries` — Seeded via `data/billing_inquiry_summaries.csv`

---

## 5. Example Questions

1. "What are our primary operational metrics and performance targets for multi line account management across operating regions in 2026 YTD?"
2. "What are the latest telecom industry standards, GSMA guidelines, and market benchmarks for multi line account management?"
3. "Render a chart comparing monthly performance metrics for multi line account management vs annual targets."
4. "Break down multi line account management volume by operating cluster and customer segment for 2026 YTD."
5. "What are the projected quarterly financial impacts and ROI of optimizing multi line account management?"
...
```bash
# Run unit tests
uv run --frozen pytest domains/subscriber_crm/agents/multi_line_account_management/tests/unit -v

# Run interactively with ADK CLI
adk run domains/subscriber_crm/agents/multi_line_account_management
```
