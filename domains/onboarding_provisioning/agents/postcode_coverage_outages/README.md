...
### Business Problem
Addressing core telecommunications operational challenges in postcode coverage outages through automated quantitative analytics, predictive AI, and real-time grounding.
...
- **`data_insights`**: Queries internal BigQuery tables (`onpr_pcoo_*`) using the BigQuery Conversational Analytics API (`ask_data_insights`, `forecast`, `analyze_contribution`, `detect_anomalies`) and generates visual data charts via `render_chart`.
...
- `onpr_pcoo_address_geocodes` — Seeded via `data/address_geocodes.csv`
- `onpr_pcoo_cell_coverage_ratios` — Seeded via `data/cell_coverage_ratios.csv`
- `onpr_pcoo_planned_outage_schedules` — Seeded via `data/planned_outage_schedules.csv`

---

## 5. Example Questions

1. "What are our primary operational metrics and performance targets for postcode coverage outages across operating regions in 2026 YTD?"
2. "What are the latest telecom industry standards, GSMA guidelines, and market benchmarks for postcode coverage outages?"
3. "Render a chart comparing monthly performance metrics for postcode coverage outages vs annual targets."
4. "Break down postcode coverage outages volume by operating cluster and customer segment for 2026 YTD."
5. "What are the projected quarterly financial impacts and ROI of optimizing postcode coverage outages?"
...
```bash
# Run unit tests
uv run --frozen pytest domains/onboarding_provisioning/agents/postcode_coverage_outages/tests/unit -v

# Run interactively with ADK CLI
adk run domains/onboarding_provisioning/agents/postcode_coverage_outages
```
