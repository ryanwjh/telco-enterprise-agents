...
### Business Problem
Addressing core telecommunications operational challenges in mnp number porting tracker through automated quantitative analytics, predictive AI, and real-time grounding.
...
- **`data_insights`**: Queries internal BigQuery tables (`onpr_mnpt_*`) using the BigQuery Conversational Analytics API (`ask_data_insights`, `forecast`, `analyze_contribution`, `detect_anomalies`) and generates visual data charts via `render_chart`.
...
- `onpr_mnpt_donor_network_responses` — Seeded via `data/donor_network_responses.csv`
- `onpr_mnpt_porting_clearinghouse_logs` — Seeded via `data/porting_clearinghouse_logs.csv`
- `onpr_mnpt_activation_sla_metrics` — Seeded via `data/activation_sla_metrics.csv`

---

## 5. Example Questions

1. "What are our primary operational metrics and performance targets for mnp number porting tracker across operating regions in 2026 YTD?"
2. "What are the latest telecom industry standards, GSMA guidelines, and market benchmarks for mnp number porting tracker?"
3. "Render a chart comparing monthly performance metrics for mnp number porting tracker vs annual targets."
4. "Break down mnp number porting tracker volume by operating cluster and customer segment for 2026 YTD."
5. "What are the projected quarterly financial impacts and ROI of optimizing mnp number porting tracker?"
...
```bash
# Run unit tests
uv run --frozen pytest domains/onboarding_provisioning/agents/mnp_number_porting_tracker/tests/unit -v

# Run interactively with ADK CLI
adk run domains/onboarding_provisioning/agents/mnp_number_porting_tracker
```
