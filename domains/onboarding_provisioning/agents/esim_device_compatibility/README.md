...
### Business Problem
Addressing core telecommunications operational challenges in esim device compatibility through automated quantitative analytics, predictive AI, and real-time grounding.
...
- **`data_insights`**: Queries internal BigQuery tables (`onpr_esdc_*`) using the BigQuery Conversational Analytics API (`ask_data_insights`, `forecast`, `analyze_contribution`, `detect_anomalies`) and generates visual data charts via `render_chart`.
...
- `onpr_esdc_device_eid_database` — Seeded via `data/device_eid_database.csv`
- `onpr_esdc_esim_activation_matrix` — Seeded via `data/esim_activation_matrix.csv`
- `onpr_esdc_digital_onboarding_sessions` — Seeded via `data/digital_onboarding_sessions.csv`

---

## 5. Example Questions

1. "What are our primary operational metrics and performance targets for esim device compatibility across operating regions in 2026 YTD?"
2. "What are the latest telecom industry standards, GSMA guidelines, and market benchmarks for esim device compatibility?"
3. "Render a chart comparing monthly performance metrics for esim device compatibility vs annual targets."
4. "Break down esim device compatibility volume by operating cluster and customer segment for 2026 YTD."
5. "What are the projected quarterly financial impacts and ROI of optimizing esim device compatibility?"
...
```bash
# Run unit tests
uv run --frozen pytest domains/onboarding_provisioning/agents/esim_device_compatibility/tests/unit -v

# Run interactively with ADK CLI
adk run domains/onboarding_provisioning/agents/esim_device_compatibility
```
